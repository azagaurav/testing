...
public static void main(String[] args) {
		...
		try {
			...
			Font font2 = FontFactory.getFont(FontFactory.TIMES_ROMAN, 16,Font.BOLD);
			Font font3 = FontFactory.getFont(FontFactory.TIMES_ROMAN, 11);
			Font font4 = FontFactory.getFont(FontFactory.TIMES_BOLD, 11,Font.UNDERLINE);
			Font fonthd = FontFactory.getFont(FontFactory.TIMES_ROMAN, 9,Font.BOLD);
			Font fonthd1 = FontFactory.getFont(FontFactory.TIMES_ROMAN, 9,Font.BOLD);

			docum = new Document(PageSize.A4, 20, 20, 20, 20);
			//String[] emp = empid.split("-");
			//System.out.println("d:\\" + emp[0] + "\\Form 2\\" + empid +
			// ".pdf");
			final String pdfCreationPath = "D:"+File.separator+"SLIPS"+File.separator;
			PdfWriter.getInstance(docum, new FileOutputStream(pdfCreationPath+"Form2.pdf"));
			docum.open();
